...
public class LipSyncer3D : LipSyncer
{
    ...
    public override void GenerateAnimation()
    {
        ...
        print(LipSyncConstants.MouthShape.Count);
        ...
        foreach (string s in mouthLines)
        {
            if (!s.Trim().Equals(""))
            {
                ...
                currentIndex = 0;//LipSyncConstants.MouthShape.FindIndex(param[1].Contains);
                if (!s.Trim().Equals("X"))
                {
        ...
        for (int i = 0; i < mouthKeyframe.Length - 1; i++)
        {
            ...
            {
                print(LipSyncConstants.MouthShape[i]);
                print("asdasd");
                resultAnimation.SetCurve("", typeof(SkinnedMeshRenderer), "blendShape." + LipSyncConstants.MouthShape[i], curve);
            }
        }
